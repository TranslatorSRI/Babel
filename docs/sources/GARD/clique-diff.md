...
either upstream correction or a bad-xrefs entry, and is tracked separately from #1032 as
[#1064](https://github.com/NCATSTranslator/Babel/issues/1064). Neither kind is fixed by this PR, and
both would re-fuse their cliques if MONDO stopped mapping the diseases to different GARD terms.

The per-edge record for all of them, including the 19 shape-1 rows this section does not walk, is
[`docs/sources/DOID/cross-disease-xrefs.csv`](../DOID/cross-disease-xrefs.csv) — 29 crossing rows
across 11 over-merges, regenerated by `find_cross_disease_xrefs.py`. Read it as a worklist rather
than a verdict: a crossing row is sometimes the correct xref, with the *other* endpoint being the
identifier in the wrong clique. `DOID:8712` "neurofibromatosis" → `MESH:D017253` "Neurofibromatoses"
is right; what is wrong is that the MeSH term is in the neurofibroma clique.
